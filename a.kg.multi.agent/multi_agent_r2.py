...
async def main():
    ...
    report_generator = AssistantAgent(
        ...
        system_message="""
            ...
            - 不参与分析，仅组织他们的结论；
            - 如果有降本增效、安全、稳定性建议等，必须在报告中明确列出相关的可操作对象，比如具体主机型号、pod、域名等；
    ...
    team = SelectorGroupChat(
        ...
        selector_prompt = """
            ...
            5.  **报告生成阶段**:
                * **前提条件**: 只有当所有分析专家（cost_analyst、stability_analyst、security_analyst）最近一次回复的最后一行均为“任务完成：True”或者"任务终止"，才允许选择 report_generator 进行综合汇总。
